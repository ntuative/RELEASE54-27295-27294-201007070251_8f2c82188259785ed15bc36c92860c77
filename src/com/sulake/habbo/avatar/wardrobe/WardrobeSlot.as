package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_318 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_420 && var_1287)
         {
            _loc5_ = _controller.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_626,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_30,parseInt(FigureData.const_538));
         ...
         if(var_318)
         {
            if(false)
            {
               var_318.bitmap.dispose();
            }
            var_318.bitmap = new BitmapData(var_318.width,var_318.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_318.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_420 = _controller.figureData.getFigureString();
               var_626 = _controller.gender;
               _controller.handler.saveWardrobeOutfit(var_1563,this);
               updateView();
               break;
            case "get_button":
               if(var_420)
               {
                  _controller.loadAvatarInEditor(var_420,var_626,_controller.clubMemberLevel);
               }
         }
      }
