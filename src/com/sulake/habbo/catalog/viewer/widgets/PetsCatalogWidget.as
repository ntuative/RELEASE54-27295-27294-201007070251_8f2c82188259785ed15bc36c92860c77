package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_463 >= var_333.length)
         {
            return "";
         }
         var petColor:IPetColor = var_333[var_463] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_670[var_1676] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_463 = param1.index;
         if(var_463 < 0 || var_463 > var_333.length)
         {
            var_463 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_634.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_979 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_333 = [];
         var_670 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_979,this);
               if(_loc11_)
               {
                  var_670 = _loc11_.breeds;
                  var_333 = _loc11_.colors;
                  if(var_333 == null)
                  {
                     var_333 = [];
                  }
         ...
         {
            _loc12_ = var_670[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_979,_loc12_.id),getRaceLocalizationKey(var_979,_loc12_.id)));
            _loc6_++;
         }
         var_634.add(var_979,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1048,onApproveNameResult);
         _loc5_ = var_634.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_333)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_634.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_463 >= 0 && var_463 < var_333.length)
         {
            _loc7_ = var_333[var_463] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_670[var_1676] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_979,_loc3_.id,_loc2_,AvatarScaleType.const_49,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_281,AvatarAction.const_603);
