package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1450:uint = 2;
      
      protected static const const_1449:uint = 1;
      ...
      protected static const const_1448:uint = 3;
      ...
      {
         var_2417 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_155,bytesTotal,bytesLoaded,elapsedTime));
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,var_155,bytesTotal,bytesLoaded,elapsedTime));
            try
            {
               var_17.close();
               var_17.unload();
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_385 = this.getDefinition(_name) as Class;
         if(var_385 == null)
         {
         ...
         try
         {
            xmlClass = var_385.manifest as Class;
            if(xmlClass == null)
            {
               return;
            }
         ...
         var_565 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(var_694)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_155,bytesTotal,bytesLoaded,elapsedTime));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               var_155 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_155 + " for file \"" + var_17.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_17.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_155,bytesTotal,bytesLoaded,elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_17.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_155,bytesTotal,bytesLoaded,elapsedTime));
               break;
         ...
         if(var_479 == LibraryLoader.const_1448)
         {
            if(analyzeLibrary())
            {
               var_479 |= 0;
         ...
         if(var_479 == LibraryLoader.name_2)
         {
            ...
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_155,bytesTotal,bytesLoaded,elapsedTime));
         }
      }
      ...
      {
         return !!var_587 ? uint(var_194 - var_835) : uint(getTimer() - var_835);
      }
