   ...
   public class ComponentContext extends Component implements IContext
   {
      ...
      protected final function announceInterfaceAvailability(param1:IID, param2:Component) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            if(_loc4_ == null)
            {
               error("Interface " + param1 + " still unavailabe!",true,Core.const_1269);
            }
      ...
      override public function toXMLString(param1:uint = 0) : String
      {
         ...
         var _loc11_:int = 0;
         while(_loc11_ < var_304.length)
         {
            _loc10_ = var_304[_loc11_] as Component;
            if(_loc10_ != this)
            {
      ...
      protected function loadErrorHandler(param1:LibraryLoaderEvent) : void
      {
         ...
         removeLibraryLoader(_loc2_);
         error("Failed to download component resource \"" + _loc2_.url + "\"!" + "\r" + _loc3_,true,Core.const_1417);
      }
      ...
      public final function detachComponent(param1:Component) : void
      {
         ...
         {
            if(var_304[_loc4_] == param1)
            {
               var_304.splice(_loc4_,1);
               return;
            }
            _loc4_++;
         }
         ...
         while(i < componentList.length())
         {
            ...
            if(assetCount > 0)
            {
               ...
               while(k < assetCount)
               {
                  ...
                  {
                     error("Manifest for component " + componentString + " contains broken asset \"" + assetName + "\"!",true,Core.const_580);
                  }
                  k++;
               }
            }
            componentString = item.attribute("RoomObjectDimmerStateUpdateEvent");
            ...
            if(componentClass == null)
            {
               error("Invalid component class " + componentString + "!",true,Core.const_580);
               return false;
            }
            ...
            if(component != null)
            {
               if(assetLibrary != null)
               {
                  if(component.assets != assetLibrary)
                  {
                     ...
                     error("Component \"" + componentString + "\" did not save provided asset library!",true,Core.const_580);
                  }
               }
